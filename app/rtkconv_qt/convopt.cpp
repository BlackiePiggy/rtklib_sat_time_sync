...
void ConvOptDialog::showEvent(QShowEvent *event)
{
    ...
    Freq5->setChecked(mainWindow->FreqType&FREQTYPE_E5ab);
    Freq6->setChecked(mainWindow->FreqType&FREQTYPE_S);
    Freq7->setChecked(mainWindow->FreqType&FREQTYPE_ALL);
...
void ConvOptDialog::BtnOkClick()
{
    ...
    if (Freq5->isChecked()) freqtype|=FREQTYPE_E5ab;
    if (Freq6->isChecked()) freqtype|=FREQTYPE_S;
    if (Freq7->isChecked()) freqtype|=FREQTYPE_ALL;
...
{    
    ...
    if (Freq5->isChecked()) codeOptDialog->FreqType|=FREQTYPE_E5ab;
    if (Freq6->isChecked()) codeOptDialog->FreqType|=FREQTYPE_S;
    if (Freq7->isChecked()) codeOptDialog->FreqType|=FREQTYPE_ALL;
